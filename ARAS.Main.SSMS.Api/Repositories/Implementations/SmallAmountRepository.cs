...
{
	public class SmallAmountRepository :
		BaseAdjustmentCommandService<SmallAmountRowDto, SmallAmountCreateDto, object>,
		ISmallAmountRepository
	{
		public SmallAmountRepository(
			IBaseAdjustmentRepository<SmallAmountCreateDto> baseAdjustmentRepo,
			IAdjustmentRepository adjustmentRepo,
			IInvoiceRepository invoiceRepo) :
			base("sar", Map, baseAdjustmentRepo, adjustmentRepo, invoiceRepo)
		{
		}

		public async Task<long> CreateAsync(RequestCreationDto<AdjustmentRequestCreationDto<SmallAmountCreateDto>> data, string createdBy)
		{
			ArgumentNullException.ThrowIfNull(data, nameof(SmallAmountCreateDto));

			return await _baseAdjustmentRepo.Create(data, createdBy, AdjustmentTypeCode, CreateInvoiceAdjustmentCallBack(createdBy));
		}

		public async Task<long> UpdateAsync(long requestId, RequestCreationDto<AdjustmentRequestCreationDto<SmallAmountCreateDto>> data, string modifiedBy)
		{
			ArgumentNullException.ThrowIfNull(data, nameof(SmallAmountCreateDto));
			Guards.ThrowInvalidOperationIf(!data.Model.Adjustments.Any(), Exceptions.EMPTY_CASHDISCOUNT_ROWS);

			return await _baseAdjustmentRepo.Update(requestId, data, modifiedBy, AdjustmentTypeCode, "small-amount", CreateInvoiceAdjustmentCallBack(modifiedBy));
		}
		...
		private static Func<RequestAdjustmentsV, SmallAmountRowDto> Map = (requestAdjustment) =>
			new SmallAmountRowDto()
			{
		...
		private Func<SmallAmountCreateDto, string, long, Task> CreateInvoiceAdjustmentCallBack(string createdBy)
		{
		...
		private async Task CreateInvoiceAdjustments(string createdBy, long requestId, string adjustmentTypeId, SmallAmountCreateDto data)
		{
